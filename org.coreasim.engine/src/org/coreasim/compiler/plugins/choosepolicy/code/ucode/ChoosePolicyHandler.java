package org.coreasim.compiler.plugins.choosepolicy.code.ucode;
...
import org.coreasim.engine.plugins.choosepolicy.ChoosePolicyNode;

/**
 * Handles the choose policy
 * @author Spellmaker
 *
 */
public class ChoosePolicyHandler implements CompilerCodeHandler {

	@Override
	public void compile(CodeFragment result, ASTNode node, CompilerEngine engine)
			throws CompilerException {
		try {
			if(!(node instanceof ChoosePolicyNode)) throw new CompilerException("wrong node type in compilation for choosepolicy");
			ChoosePolicyNode choosePolicy = (ChoosePolicyNode) node;
			
			Map<String, ASTNode> vars = choosePolicy.getVariableMap();
			ASTNode doPolicy = choosePolicy.getDoPolicy();
			ASTNode condition = choosePolicy.getCondition();
			ASTNode ifnone = choosePolicy.getIfnonePolicy();
			...
			result.appendFragment(engine.compile(doPolicy, CodeType.U));
